...
# ----- Base Page -----
# This class in charge of the colour palette that is carried out through the app
class BasePage(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        # All styling colours used for text, error background etc.
        style = ttk.Style()
        ...
        # Logo Placement
        # This insures that the logo is in the correct place on every page for aesthetic purposes
        try:
...
class ClientInfo(BasePage):
    ...
    def validate_fields(self):
        ...
        stripped_phone = re.sub(r'\D', '', user_part) #this is the strip function
        ...
        # I have done with through using a pattern which checks the email
    ...
    def show_summary(self):
        summary_frame = self.controller.frames["BookingSummary"]
        summary_frame.display_all_bookings()
        self.controller.show_frame("BookingSummary")
...
class SeatSelection(BasePage):
    ...
    def get_availability_text(self):
        ...
        if route == "3":  # Return
            seats1 = self.route_seats["1"]
            seats2 = self.route_seats["2"]

            recliner_avail = min(seats1['recliners'], seats2['recliners'])
            bunk_avail = min(seats1['bunks'], seats2['bunks'])

            return (f"Available Recliners: {recliner_avail}\n"
                    f"Available Bunks: {bunk_avail}")
    ...
    # This function what is in charge of updating the seats avaliablity
    # And the validation of the inputs in the seat selection section
    def submit_selection(self):
        self.clear_error_styles()
        route = self.route_var.get()

        recliner_text = self.recliner_entry.get().strip()
        bunk_text = self.bunk_entry.get().strip()

        try:
            recliners = int(recliner_text) if recliner_text else 0
            bunks = int(bunk_text) if bunk_text else 0
        except ValueError:
            self.avail_label.configure(
                text="Invalid input: Please enter whole numbers only.",
                style="Error.TLabel"
            )
            return
        ...
        if route == "3":  # Return
            ...
            if (recliners > min(seats1["recliners"], seats2["recliners"]) or
                    bunks > min(seats1["bunks"], seats2["bunks"])):
                self.avail_label.configure(
                    text="Invalid input: Not enough seats available for return trip.",
                    style="Error.TLabel"
                )
                return
        else:  # One-way
            ...
            if recliners > seats["recliners"] or bunks > seats["bunks"]:
                self.avail_label.configure(
                    text="Invalid input: Not enough seats available.",
                    style="Error.TLabel"
                )
                return
    ...
    # After the confirm button is pressed the inputs are reset for the next user
    def reset_selection(self):
        ...
        # Just refresh the availability label
        self.update_availability_label()
...
class BookingPreview(BasePage):
    def __init__(self, parent, controller):
        ...
        # Configure rows above button to expand
        for r in range(self.current_row, self.current_row + 10):
        ...
        # Confirm now saves booking before going to confirmation page
        confirm_button = ttk.Button(button_frame, text="CONFIRM", command=self.confirm_booking, width=15)
    ...
    def edit_booking(self):
        ...
        if seat_selection_frame:
            ...
            if data:
                ...
                # Pre-fill inputs only (no adding seats back!)
                if route_key:
                    seat_selection_frame.route_var.set(route_key)
    ...
    def confirm_booking(self):
        ...
        # Deduct seats only now
        if route_key == "3":  # Return trip
            seat_selection_frame.route_seats["1"]["recliners"] -= recliners
            seat_selection_frame.route_seats["1"]["bunks"] -= bunks
            seat_selection_frame.route_seats["2"]["recliners"] -= recliners
            seat_selection_frame.route_seats["2"]["bunks"] -= bunks
        elif route_key:
            seat_selection_frame.route_seats[route_key]["recliners"] -= recliners
            seat_selection_frame.route_seats[route_key]["bunks"] -= bunks

        # Save to all bookings
        self.controller.all_bookings.append(data.copy())
...
class ConfirmationPage(BasePage):
    def __init__(self, parent, controller):
        ...
        # Make columns stretch horizontally
        self.main_frame.columnconfigure(0, weight=1)
...
class BookingSummary(BasePage):
    def __init__(self, parent, controller):
        ...
        # Create window inside canvas and save ID
        self.scrollable_frame_window = self.canvas.create_window(
        ...
        # Force width to follow canvas width
        self.canvas.bind(
            "<Configure>",
            lambda e: self.canvas.itemconfig(self.scrollable_frame_window, width=e.width)
        )

        self.canvas.configure(yscrollcommand=self.scrollbar.set)

        self.canvas.grid(row=self.current_row, column=0, sticky="nsew")
        self.scrollbar.grid(row=self.current_row, column=1, sticky="ns")
        self.main_frame.grid_rowconfigure(self.current_row, weight=1)

        # GO HOME Button
        self.current_row += 1
        home_button = ttk.Button(self.main_frame, text="GO HOME", command=self.go_home)
        home_button.grid(row=self.current_row, column=0, pady=20)

        self.main_frame.grid_columnconfigure(0, weight=1)

    def display_all_bookings(self):
        # Clear old booking labels
        for lbl in self.labels:
            lbl.destroy()
        self.labels.clear()

        if not self.controller.all_bookings:
            lbl = ttk.Label(
            ...
            lbl.grid(row=0, column=0, pady=5, padx=10, sticky="ew")
            self.scrollable_frame.grid_columnconfigure(0, weight=1)
            self.labels.append(lbl)
            return

        row = 0
        for idx, data in enumerate(self.controller.all_bookings, start=1):
            recliners = data.get("recliners", 0)
            bunks = data.get("bunks", 0)
            total_cost, gst_portion = calculate_cost(recliners, bunks)
            ...
            for line in booking_lines:
                ...
                # Expand across full width, text inside stays centered
                lbl.grid(row=row, column=0, pady=2, sticky="ew")
                self.labels.append(lbl)
                row += 1

            # Make sure column stretches full width
            self.scrollable_frame.grid_columnconfigure(0, weight=1)

    def go_home(self):
        self.controller.booking_data.clear()
        client_info_frame = self.controller.frames.get("ClientInfo")
        if client_info_frame:
            client_info_frame.reset_fields()
        seat_selection_frame = self.controller.frames.get("SeatSelection")
        if seat_selection_frame:
            seat_selection_frame.reset_selection()
...
class MainApp(tk.Tk):
    def __init__(self):
        ...
        # To Save all previous bookings
        self.all_bookings = []

        for F in (ClientInfo, SeatSelection, BookingPreview, ConfirmationPage, BookingSummary):
