def validate_seat_numbers(recliners, bunks, available_recliners, available_bunks):
    if recliners < 0 or bunks < 0:
        return False, "Invalid response: we cannot accept negative numbers. Please try again :)"
...
def main():
    ...
    while True:
        ...
        # Ask for valid seat numbers
        while True:
            try:
                recliners_requested = int(input("How many recliner seats would you like to book (max:20)? "))
                bunks_requested = int(input("How many bunk beds would you like to book (max: 15)? "))
            except ValueError:
